...
export default function CreateRecipe() {
  ...
  const importRecipe = async (recipeUrl: string) => {
    ...
    try {
      ...
      setSteps(data.instructions.map((instruction: any) => ({
        text: instruction,
        expanded: false,
        id: Date.now().toString() + Math.random().toString(36).substring(7) // Ensure unique IDs
      })));
  ...
  const [steps, setSteps] = useState<{ text: string; expanded: boolean; id: string }[]>([]);
  ...
  // Updated to use ID instead of index
  ...
  // Function to handle reordering steps
  const onStepReorder = ({ data }: { data: typeof steps }) => {
    setSteps(data);
  };

  // Define the type for your step items
type StepItem = {
  text: string;
  expanded: boolean;
  id: string;
...
// Updated renderStepItem that doesn't rely on the index parameter
const renderStepItem = ({ 
  ...
  // Find the position of this item in the steps array
  const stepNumber = steps.findIndex(step => step.id === item.id) + 1;
  
  return (
    <ScaleDecorator>
      <View className={`rounded-2xl bg-background-50 p-4 mb-3 ${isActive ? 'border border-primary' : ''}`}>
        <View className="flex-row justify-between items-center">
          ...
          >
            ...
            <Icon as={item.expanded ? ChevronUpIcon : ChevronDownIcon} className="text-white ml-2" />
          </TouchableOpacity>
                ...
                <VStack space="4xl">
                  ...
                  <VStack space="md">
                    ...
                    {steps.length > 0 && (
                      ...
                      <View 
                        className="rounded-2xl bg-background-0 mt-2"
